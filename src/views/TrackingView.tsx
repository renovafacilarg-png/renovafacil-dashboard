...
export function TrackingView() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-foreground">Tracking de Envíos</h1>
      ...
      {tracking && tracking.success && tracking.data && (
        <div className="bg-card border border-border rounded-xl p-6">
          ...
          <div className="flex items-start justify-between mb-5">
            <div>
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">Estado actual</p>
          ...
          <div className="bg-muted/50 rounded-xl p-4 mb-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-muted-foreground mb-0.5">Número de tracking</p>
                <code className="text-sm font-mono text-foreground">{tracking.data.tracking_number}</code>
              </div>
          ...
          {tracking.data.eventos && tracking.data.eventos.length > 0 && (
            <div>
              <button
                ...
                className="flex items-center gap-2 text-sm font-medium text-foreground hover:text-foreground/80 mb-4"
              >
      ...
      {tracking && !tracking.success && (
        <div className="bg-card border border-destructive/50 rounded-xl p-5">
          <div className="flex items-start gap-3">
            <AlertCircle className="h-5 w-5 text-destructive mt-0.5 flex-shrink-0" />
            <div>
              <h3 className="text-sm font-medium text-foreground">No se encontró el envío</h3>
              <p className="text-sm text-muted-foreground mt-0.5">{tracking.mensaje}</p>
            </div>
          </div>
        </div>
      )}
